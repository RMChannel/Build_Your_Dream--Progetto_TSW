...
<html>
<head>
    <title>Gestione Headset - Admin</title>
...
<div id="popup-nuovo-headset" class="popup-overlay" style="display:none;">
    <div class="popup-content enhanced-popup">
        <span class="close-btn" onclick="closePopup()">&times;</span>
        <h3>Nuovo Headset</h3>
        <form id="form-nuovo-headset" action="<%=request.getContextPath()%>/newProduct" method="post" enctype="multipart/form-data">
            ...
            <input type="hidden" name="name" value="HeadSet" />
          ...
          <div class="popup-fields-row">
            <div class="popup-field-wrapper">
              <label for="microfono-new" class="visually-hidden">Microfono</label>
              <select id="microfono-new" name="microfono" required aria-label="Microfono">
            ...
            <div class="popup-field-wrapper">
              ...
              <input type="text" id="connectionType-new" name="connectionType" required aria-label="Tipo Connessione" placeholder="Es: USB, Jack 3.5mm">
            </div>
          </div>
          <div class="popup-fields-row">
            <div class="popup-field-wrapper">
...
<h2 class="admin-title">Gestione Headset</h2>
<div class="admin-table-container">
    <table class="admin-table">
        ...
        <tbody>
        <c:forEach var="headset" items="${headsets}">
            <tr>
                <form class="update-form" action="<%=request.getContextPath()%>/updateProduct" method="post">
                    <td><input type="hidden" name="id" value="${headset.ID}" />${headset.ID}</td>
                    <td>
                        <label for="marca-${headset.ID}" class="visually-hidden">Marca</label>
                        <input type="text" id="marca-${headset.ID}" name="marca" value="${headset.marca}" aria-label="Marca" />
                    </td>
                    <td>
                        <label for="modello-${headset.ID}" class="visually-hidden">Modello</label>
                        <input type="text" id="modello-${headset.ID}" name="modello" value="${headset.modello}" aria-label="Modello" />
                    </td>
                    <td>
                        <label for="prezzo-${headset.ID}" class="visually-hidden">Prezzo</label>
                        <input type="number" id="prezzo-${headset.ID}" name="prezzo" value="${headset.prezzo}" step="0.01" min="0" aria-label="Prezzo" />
                    </td>
                    <td>
                        <label for="disponibilita-${headset.ID}" class="visually-hidden">Disponibilità</label>
                        <input type="number" id="disponibilita-${headset.ID}" name="disponibilita" value="${headset.disponibilita}" min="0" aria-label="Disponibilità" />
                    </td>
                    <td>
                        <label for="sconto-${headset.ID}" class="visually-hidden">Sconto</label>
                        <input type="number" id="sconto-${headset.ID}" name="sconto" value="${headset.sconto}" min="0" max="100" aria-label="Sconto percentuale" />
                    </td>
                    <td>
                        <label for="categoria-${headset.ID}" class="visually-hidden">Categoria</label>
                        <input type="text" id="categoria-${headset.ID}" name="categoria" value="${headset.categoria}" aria-label="Categoria" />
                    </td>
                    <td>
                        <label for="microfono-${headset.ID}" class="visually-hidden">Microfono</label>
                        <select id="microfono-${headset.ID}" name="microfono" aria-label="Microfono">
                            <option value="true" ${headset.microfono ? 'selected' : ''}>Sì</option>
                            <option value="false" ${!headset.microfono ? 'selected' : ''}>No</option>
                        </select>
                    </td>
                    <td>
                        <label for="connectionType-${headset.ID}" class="visually-hidden">Tipo Connessione</label>
                        <input type="text" id="connectionType-${headset.ID}" name="connectionType" value="${headset.connectionType}" aria-label="Tipo Connessione" />
                    </td>
                    <td>
                        <label for="led-${headset.ID}" class="visually-hidden">LED</label>
                        <select id="led-${headset.ID}" name="led" aria-label="LED">
                            <option value="true" ${headset.led ? 'selected' : ''}>Sì</option>
                            <option value="false" ${!headset.led ? 'selected' : ''}>No</option>
                        </select>
                    </td>
                    <td>
                        ...
                        %>
                        <img src="<%=request.getContextPath()%>/<%=imagePath%>" alt="${headset.marca}_${headset.modello}" width="80">
                    </td>
                    <td>
                        <input type="hidden" name="piece" value="accessorio" />
                        <input type="hidden" name="type" value="headset" />
                ...
                <td colspan="12" style="padding:0; border:none;">
                    <form action="<%=request.getContextPath()%>/updateFotoProduct" method="post" enctype="multipart/form-data" style="margin-top:8px; display:inline-block;">
                        ...
                        <input type="hidden" name="category" value="fotoAccessori" />
                        <input type="hidden" name="name" value="HeadSet" />
                        <label for="foto-upload-${headset.ID}" class="visually-hidden">Carica foto</label>
                        <input type="file" id="foto-upload-${headset.ID}" name="foto" accept="image/*" style="margin-bottom:4px;" required aria-label="Carica nuova foto" />
...
<script>
    ...
    window.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById("form-nuovo-headset").addEventListener("submit", newProduct);
    ...
    function openPopup() {
        document.getElementById('popup-nuovo-headset').style.display = 'flex';
    }

    function closePopup() {
        document.getElementById('popup-nuovo-headset').style.display = 'none';
    }
</script>
</html>
